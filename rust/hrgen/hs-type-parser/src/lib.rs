  ...
  I: combine::Stream<Item = char>,
{
  ...
  fn parse_lazy(&mut self, input: I) -> combine::ConsumedResult<Type, I> {
    ...
    // Terminals
    let var_parser = try(
      lex((lower(), many(satisfy(is_ident_tail))))
        .map(char_join)
        .map(var),
    );
    let constr_parser = try(
      lex((upper(), many(satisfy(is_ident_tail))))
        .map(char_join)
        .map(constr),
    );
    ...
    let op_parser = arrow
      .map(Some)
      .or(skip_many1(space()).map(|_| None))
      .map(|op| match op {
        Some(_) => (
          op,
          Assoc {
    ...
    let mut expr = expression_parser(term, op_parser, |l, o, r| match o {
      Some(op) => app2(op, l, r),
      None => app(l, r),
    });
  ...
  S: Selector<(L::Output, R::Output)>,
{
  ...
  fn parse_lazy(
    &mut self,
    input: Self::Input,
  ) -> combine::ConsumedResult<Self::Output, Self::Input> {
...
fn parse_type_test() {
  ...
  assert_eq!(
    parse_type("In a -> (In b -> Out) -> Out"),
    Ok((
      func(
        app(constr("In"), var("a")),
        func(
          func(app(constr("In"), var("b")), constr("Out")),
          constr("Out")
        )
      ),
      ""
  ...
  assert_eq!(
    parse_type("FunPtr (I32 -> IO I32) -> IO ()"),
    Ok((
      func(
        app(
          constr("FunPtr"),
